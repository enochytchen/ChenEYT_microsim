## Filename: 07_fullmod_fpm_ac3_PSA
## Purpose: Run microsimulation model for probabilistic sensitivity analysis
##          All-cause survival framework
##          Using very simple models with time-dependent effect 
##          (flexible parametric models with df == 1, weibull for all transitions,
##                                           dftvc == 2)
...
# I revised the codes from above to get basically a weibull distribution for every model
# with dftvc
    ...
    m1_fpm <-  stpm2(Surv(Tstart, Tstop, status == 1) ~ treat, data = msmcancer1, df = 1,
                     tvc = list(treat = 3))
    AIC(m1_fpm)
    summary(m1_fpm)
    ...
    ##============================================================
    ##############################################################
    m2_fpm_ac <- stpm2(Surv(Tstart, Tstop, status == 1) ~ treat, 
                       data = msmcancer2, df = 1,
                       tvc = list(treat = 3))
    ...
    ##============================================================
    ##############################################################
    m3_semiMarkov_fpm_ac <- stpm2(Surv(time, status == 1) ~ treat, data = msmcancer3,
                                  df = 1,
                                  tvc = list(treat = 3))
...
# save(output_df, file = "../Output/07_fullmod_fpm_ac3_PSA.RData")
################################################################
