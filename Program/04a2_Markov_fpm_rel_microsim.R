## Filename: 04a2_Markov_fpm_rel_microsim
  ...
  */
  void IDM::handleMessage(const ssim::cMessage* msg) {
    ...
    case toProg:
      ...
      scheduleAt(m3.randU(R::runif(0.0,1.0), now()), toExcD); // Caution: delayed entry, clock-forward
      scheduleAt(background.rand(std::exp(-R::rexp(1.0)), dxage+now())-dxage, toExpD);
      break;
    case toExcD:
      state = ExcD;
...
results <- lapply(treat_values, function(treat_value) {
    ...
    param <- list(treat = treat_value,
                  ...
                  m3 = gsm_design(m3_Markov_fpm_rel, newdata = ndata))
...
# saveRDS(results_FC, file = "../Data/04a2_Markov_fpm_rel_microsim_FC.rds")
# saveRDS(results_RFC, file = "../Data/04a2_Markov_fpm_rel_microsim_RFC.rds")
################################################################
