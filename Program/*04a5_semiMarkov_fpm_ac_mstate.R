## Filename: 04a5_extrap_semiMarkov_fpm
...
crfs <- fmsm(m1_flexfpm_ac, m2_flexfpm_ac, m3_semiMarkov_flexfpm_ac, trans = tmat)
...
fpm1fpm2fpm3sim_RFC<- mssample(ms_RFC$Haz, trans = tmat, clock = "reset", M = 1e5,
                               tvec =  seq(0, 50, by = 0.1))
fpm1fpm2fpm3sim_FC<- mssample(ms_FC$Haz, trans = tmat, clock = "reset", M = 1e5,
                              tvec =  seq(0, 50, by = 0.1))
...
write.table(semiMarkov_fpm, file = "../Data/04a5_semiMarkov_fpm_ac_mstate.txt", sep = " ",
